...
function DeliveryStatusBadge({ status }: { status?: string }) {
  ...
  return (
    <span className={cn('inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold capitalize', colors)}>
        ...
        <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
          <div>
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="text-xs text-slate-400">Delivery #{delivery.id}</span>
        ...
        <div className="mb-4 grid gap-3 text-sm sm:grid-cols-2">
          ...
          <div className="flex items-start gap-2.5 rounded-xl bg-slate-50 p-3">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-0.5 font-medium text-slate-800 leading-snug">{delivery.pickupAddress}</p>
            </div>
          </div>
          ...
          <div className="flex items-start gap-2.5 rounded-xl bg-slate-50 p-3">
            ...
            <div className="min-w-0">
              <p className="text-xs font-medium text-slate-400 uppercase tracking-wide">Drop-off</p>
              <p className="mt-0.5 font-medium text-slate-800 leading-snug">{delivery.deliveryAddress}</p>
            </div>
          </div>
        </div>
          ...
          >
            ...
            {status === 'DELIVERED' && (
              <div className="flex items-center gap-2 rounded-xl bg-green-50 px-4 py-2.5">
                ...
                <span className="text-sm font-semibold text-green-700">Delivery Completed</span>
              </div>
            )}
            ...
            {status === 'FAILED' && (
              <div className="flex items-center gap-2 rounded-xl bg-red-50 px-4 py-2.5">
                ...
                <span className="text-sm font-semibold text-red-700">Delivery Failed</span>
              </div>
            )}
          </motion.div>
        </AnimatePresence>
...
export function RiderDeliveriesPage() {
  ...
  const activeDeliveries = deliveries.filter(
    (d) => d.status === 'ASSIGNED' || d.status === 'PICKED_UP' || d.status === 'IN_TRANSIT' || d.status === 'PENDING' || !d.status
  )
      ...
      <>
        ...
        <AppCard>
          <div className="flex flex-col items-center gap-4 py-10 text-center">
            ...
            <div>
              <p className="font-semibold text-slate-900">Failed to load deliveries</p>
    ...
    <>
      ...
      <div className="mb-6 flex flex-wrap gap-3">
        <div className="flex items-center gap-2 rounded-xl bg-blue-50 px-3.5 py-2">
          ...
          <span className="text-sm font-medium text-blue-700">{activeDeliveries.length} active</span>
        </div>
        <div className="flex items-center gap-2 rounded-xl bg-green-50 px-3.5 py-2">
          <CheckCircle className="h-4 w-4 text-green-600" />
          <span className="text-sm font-medium text-green-700">
            {completedDeliveries.filter((d) => d.status === 'DELIVERED').length} delivered
          </span>
        </div>
        <div className="flex items-center gap-2 rounded-xl bg-slate-100 px-3.5 py-2">
          <Package className="h-4 w-4 text-slate-500" />
          <span className="text-sm font-medium text-slate-600">{deliveries.length} total</span>
        </div>
      </div>
